...
- **生产环境**: 使用 `https://yearprogress.org`
...
生产域名 (yearprogress.org) -> https://yearprogress.org  
Vercel 预览 (*.vercel.app) -> https://yearprogress.org
其他自定义域名 -> 动态检测
```
...
```bash
# 可在 .env 中设置自定义域名
NEXT_PUBLIC_SITE_URL=https://yearprogress.org
```
...
#### 生产环境域名：
```
https://yearprogress.org/robots.txt
https://yearprogress.org/sitemap.xml
https://yearprogress.org/manifest.webmanifest
```
...
5. **结构化数据**: 帮助搜索引擎理解内容
6. **生产就绪**: 使用正确的生产域名 `https://yearprogress.org`
